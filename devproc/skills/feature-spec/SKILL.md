...
1. If $ARGUMENTS looks like a GitHub issue reference — e.g. "issue 6", "issue #6", "#6", or natural language such as "the issue about improving error handling" — resolve it before proceeding:
   ...
   b. For a numeric reference: run `gh issue view N --repo owner/repo --comments` to fetch the title, body, and comments. Credentials may be unnecessary, or may be in env vars - try before assuming it will not work.
   ...
   e. If `gh` is not available or the lookup fails, tell the user and ask them to provide the description manually or to provide credentials.
