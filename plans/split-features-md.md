...
**Session summary:** Sub-tasks 1–4 done. All five skills plus the review skills, README, workflow.md, plugin.json, and the three affected agents now describe/use the `features/` layout. Workspace `CLAUDE.md` intentionally left until Sub-task 5, where the repo migration rewrites it.
**Sub-task in progress:** None (Sub-task 4 complete)
**First action next session:** Begin Sub-task 5 — migrate this repo by running the new `/feature-init` migration logic by hand (create `features/`, split `FEATURES.md`, move `plans/` → `features/plans/`, delete originals, rewrite root `CLAUDE.md`), then verify.
...
5. **Migrate this repo** — apply the new scheme here: create `features/`, split `FEATURES.md`, move `plans/*.md` into `features/plans/`, delete top-level `FEATURES.md` and `plans/`, update root `CLAUDE.md` (feature-model section, plugin descriptions, current status); verify.
